...
module.exports = {
	...
	queryParser: async function(fileString, sqlId, queryParam) {
		...
		return targetChildren ? this._recursive(targetChildren, queryParam, XmlElement) : `sqlId : [ ${sqlId} ] not found.`;
	},

	_if: function(child, queryParam) {
		return this._testCondition(child.attr.test, queryParam) ? this._recursive(child.children, queryParam) : ''
	},
		...
		for (var i = 0 ; i < collectionValue.length; i++) {
			...
			for (var j = 0; j < children.length; j++) {
				...
				if (children[j].children) {
					query = query + this._recursive(children[j].children, { [item]: collectionValue[i] });  // node, paramObject
				}
			}
